...
* 내 방의 친구들에게 돈을 뿌린다.
    ...
    * 분배할 금액을 정의한다. 차액이 발생한다면 처음 요청자가 수령하도록 분배한다.

## 2. 돈 받기
* 받기 요청한 사용자에게 분배한다.
    * 차액이 발생한다면 처음 요청한 사용자에게 지급한다. 
    * 이미 받은 사용자는 받을 수 없다.
...
```
# 핵심 문제 해결 전략
## 1. 여러 인스턴스 혹은 동일 인스턴스로 동시에 동일인 혹은 타인이 뿌리기를 받을 경우
동일인이 다른 디바이스로 동시에 받기를 요청한 경우 동시 수령 문제를 해결하기 위해 update query를 요청하는 시점에 데이터베이스에서 재확인한다.
 
1. 뿌리기 건에 할당된 것들 중에서 수령자가 없고 아직 상태가 false인 빈 영수증들을 확인해서 다른 인스턴스와 요청에서 선점했는지 확인한다.
2. 뿌리기 건에 할당된 것들 중에서 수령자에 이미 요청자의 아이디가 있는지 확인한다. 
3. 이미 선점했다면 롤백하고, 선점하지 않았다면 선점한다.
4. 나중에 들어온 요청자가 먼저 선점하는 경우를 방지하기 위해 정렬해서 모든 요청자가 순서대로 선점할 수 있게끔 한다.
     
## 2. 같은 방의 사용자라는 것을 확인할 방법
요청온 방 아이디와 토큰이 일치하다면 같은 방의 사용자만 알 수 있는 정보라고 생각하고 처리한다.

## 3. 중복 토큰이 발생할 경우
문자열 3자리를 구성하는 대문자 26개, 소문자 26개, 숫자 9개만으로도 경우의 수가 많지만 중복된 토큰이 발생할 수 있기 때문에 최대 5번 데이터베이스에서 고유한지 확인한다.
